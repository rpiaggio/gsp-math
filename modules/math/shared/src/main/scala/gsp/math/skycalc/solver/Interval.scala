...
case class Interval(start: Long, end: Long) extends Ordered[Interval] {
  ...
  def asHours: Double = duration.toDouble / Duration.ofHours(1).toMillis

  /** Gets duration of interval as days. */
  def asDays: Double = duration.toDouble / Duration.ofDays(1).toMillis
}
...
object Interval {
  ...
  def reduce(i1: Interval, i2: Interval): Seq[Interval] =
    if (i1.start < i2.start && i1.end > i2.end)
      Seq(Interval(i1.start, i2.start), Interval(i2.end, i1.end))
