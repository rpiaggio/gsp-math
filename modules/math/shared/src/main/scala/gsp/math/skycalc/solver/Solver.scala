...
/**
  ...
  */
case class DefaultSolver[A](step: Long = Duration.ofSeconds(30).toMillis) extends Solver[A] {
  require(step > 0)
  ...
  def solve(constraint: Constraint[A], interval: Interval, param: A): Solution = {
    ...
    def solve(curStart: Long, curState: Boolean, t: Long, solution: Solution): Solution =
      ...
      else if (constraint.metAt(t, param) == curState) solve(curStart, curState, t + step, solution)
...
/**
  ...
  */
case class ParabolaSolver[A](tolerance: Long = Duration.ofSeconds(30).toMillis) extends Solver[A] {
  require(tolerance > 0)
